    ...
    async setAttribute(traceName: string, attributeName: string, attributeValue: string) {
        if (await this.ensureInitialized() && this.performance) {
            try {
                ...
                console.error(`Error setting attribute "${attributeName}" for trace "${traceName}":`, error);
            }
        }
        return false;
    }
    ...
    async getAttribute(traceName: string, attributeName: string) {
        if (await this.ensureInitialized() && this.performance) {
            try {
                ...
                console.error(`Error getting attribute "${attributeName}" from trace "${traceName}":`, error);
            }
        }
        return null;
    }
